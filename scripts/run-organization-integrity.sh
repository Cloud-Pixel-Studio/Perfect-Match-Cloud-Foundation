...
(cd apps/api && uv run --no-sync python -m pmc_api.bootstrap_database && uv run --no-sync alembic upgrade head && uv run --no-sync alembic downgrade 0003_audit_engine && uv run --no-sync python ../../scripts/organization_migration_regression.py seed && uv run --no-sync alembic upgrade 0004_organization_structure && uv run --no-sync python ../../scripts/organization_migration_regression.py verify && uv run --no-sync pytest -m organization_integrity tests/test_organization_integrity.py && if uv run --no-sync alembic downgrade 0003_audit_engine; then echo 'organization audit history downgrade unexpectedly succeeded' >&2; exit 1; else echo 'organization audit history downgrade denied as designed'; fi)
